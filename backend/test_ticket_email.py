...
cur.execute("SELECT COUNT(*) as count FROM entradas")
count = cur.fetchone()['count']
print(f"Entradas actuales: {count}")

cur.execute("SELECT id, email, nombre FROM usuarios WHERE email = %s", ('guilles10@yahoo.com.ar',))
user = cur.fetchone()
usuario_id = user['id']
print(f"Usuario ID: {usuario_id}, Email: {user['email']}, Nombre: {user['nombre']}")

cur.execute("SELECT id, nombre, fecha, lugar FROM eventos ORDER BY id DESC LIMIT 1")
evento = cur.fetchone()
evento_id = evento['id']
print(f"Evento: {evento['nombre']} (ID: {evento_id})")

codigo = f'GA-{uuid.uuid4().hex[:12].upper()}'
print(f"Codigo: {codigo}")

cur.execute('''INSERT INTO entradas (evento_id, usuario_id, cantidad, total, estado, preference_id, payment_id, creada_en, usada, transferida)
               VALUES (%s, %s, %s, %s, %s, %s, %s, NOW(), 0, 0) RETURNING id''',
            (evento_id, usuario_id, 1, 5000, 'pagada', codigo, None))
entrada_id = cur.fetchone()['id']
print(f"Entrada creada ID: {entrada_id}")

cur.execute("SELECT nombre, fecha, lugar FROM eventos WHERE id = %s", (evento_id,))
evt = cur.fetchone()

nombre = user['nombre']
apellido = "DeQuiró"
email = user['email']
evento_nombre = evt['nombre']
fecha = evt['fecha']
lugar = evt['lugar']
cantidad = 1
total = 5000
...
print("\nTicket enviado a guilles10@yahoo.com.ar")